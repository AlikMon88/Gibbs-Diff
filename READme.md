...
1.  **Build the Docker Image:**
    ```bash
    docker build -t gdiff_repro .
    ```
2.  **Run the Docker Container:**
    For interactive use (e.g., Jupyter notebooks):
    ```bash
    docker run -it -p 8888:8888 -v $(pwd)/data:/app/data gdiff_repro
    ```
    (This mounts a local `data` directory into the container's `/app/data`.)
...
*(If you generate Doxygen for your code):*
A Doxygen documentation of the implemented code can be generated by running `doxygen Doxyfile` in the project root. The output will be in `docs/html/`.
