...
def normalize_phi(phi, phi_min=PHI_MIN, phi_max=PHI_MAX, mode='compact'):
    ret = (phi - phi_min) / (phi_max - phi_min)
    return ret

## de-standardize = [0, 1] -> [-1, 1]
def unnormalize_phi(phi, phi_min=PHI_MIN, phi_max=PHI_MAX, mode='compact'):
    ret = phi * (phi_max - phi_min) + phi_min
    return ret

def sample_phi_prior(n, phi_min=PHI_MIN, phi_max=PHI_MAX, norm_mode='compact'):
...
def sample_hmc_v2(log_prob_fn, log_grad_fn, phi_init, inv_mass_matrix=None, step_size=0.02, n_leapfrog_steps=50, chain_length=100, burnin_steps=20, adapt=True, n_adapt=100, phi_min_norm=None, phi_max_norm=None):
